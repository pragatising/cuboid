#!/usr/bin/env node
/**
 * Foundation CSS variables from src/theme/output/theme.json.
 *
 * theme.css exports ONLY:
 *   - globals (color palette + functional foreground/background/border)
 *   - space, border (radius/width), stack, z-index, container, breakpoints
 *   - typography, shadows, motion
 *
 * Component colors, component layout, selector rules → components.css
 */

import fs from "fs";
import path from "path";
import { fileURLToPath, pathToFileURL } from "url";
import { spaceScaleToPxKey } from "./lib/spaceScale.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");

const INPUT = path.join(ROOT, "src/theme/output/theme.json");
const OUTPUT = path.join(ROOT, "src/theme/output/theme.css");

const PREFIX = "--cube";

function cssSegment(key) {
  return String(key).replace(/[A-Z]/g, (ch) => `-${ch.toLowerCase()}`);
}

function emitNestedStringVars(obj, pathParts, prefix, set) {
  if (typeof obj === "string") {
    const name = [prefix, ...pathParts.map(cssSegment)].filter(Boolean).join("-");
    set(name, obj);
    return;
  }
  if (!obj || typeof obj !== "object") return;
  for (const [key, value] of Object.entries(obj)) {
    emitNestedStringVars(value, [...pathParts, key], prefix, set);
  }
}

function emitSpaceScaleTokens(spaceScale, set) {
  // Public spacing CSS vars, named by px count (`--cube-space-8px`,
  // `--cube-space-12px`, …) — not the "1x"/"1.5x" scale key. A raw `.` isn't
  // legal in an unescaped CSS custom-property ident, so a scale-key name
  // would silently drop the whole declaration for any fractional stop
  // (Chrome parses `--cube-space-1.5x` as invalid and discards it). The
  // *value* is untouched (still rem — see theme.json) so spacing keeps
  // scaling with the user's OS/browser text-size setting.
  if (!spaceScale || typeof spaceScale !== "object") return;
  for (const [scaleKey, value] of Object.entries(spaceScale)) {
    if (typeof value !== "string") continue;
    const pxKey = spaceScaleToPxKey(scaleKey);
    if (!pxKey) continue;
    set(`space-${pxKey}px`, value);
  }
}

function emitFoundationSizeTokens(t, set) {
  emitSpaceScaleTokens(t.sizes?.spaceScale, set);
  emitNestedStringVars(t.sizes?.borderRadius, [], "sizes-borderRadius", set);
  emitNestedStringVars(t.sizes?.borderWidth, [], "sizes-borderWidth", set);
  emitNestedStringVars(t.sizes?.stack?.gap, [], "stack-gap", set);
  emitNestedStringVars(t.sizes?.stack?.padding, [], "stack-padding", set);
  emitNestedStringVars(t.sizes?.zIndex, [], "z-index", set);
  set("breakpoint-md", t.sizes?.breakpoints?.md);
  set("breakpoint-lg", t.sizes?.breakpoints?.lg);

  const containerSizes = t.sizes?.container;
  if (containerSizes) {
    set("container-panel-minWidth", containerSizes.panel?.minWidth);
    set("container-sheet-minWidth", containerSizes.sheet?.minWidth);
    set("container-sheet-defaultWidth", containerSizes.sheet?.defaultWidth);
    set("container-sheet-maxWidth", containerSizes.sheet?.maxWidth);
    set("container-sidebar-widthSm", containerSizes.sidebar?.widthSm);
    set("container-sidebar-widthMd", containerSizes.sidebar?.widthMd);
    set("container-sidebar-widthLg", containerSizes.sidebar?.widthLg);
    set("container-sidebar-minWidth", containerSizes.sidebar?.minWidth);
    set("container-sidebar-maxWidth", containerSizes.sidebar?.maxWidth);
    set("container-tooltip-maxWidth", containerSizes.tooltip?.maxWidth);
    set("container-tooltip-maxWidthSingleLine", containerSizes.tooltip?.maxWidthSingleLine);
    set("container-popover-minWidth", containerSizes.popover?.minWidth);
    set("container-popover-maxWidth", containerSizes.popover?.maxWidth);
  }

  const iconSizes = t.sizes?.icon;
  if (iconSizes) {
    for (const stop of ["xs", "sm", "md", "lg"]) {
      set(`icon-${stop}`, iconSizes[stop]);
    }
  }
}

function emitTypographyTokens(t, set) {
  set("typography-fontFamily-base", t.typography?.fontFamily?.base);
  set("typography-fontFamily-mono", t.typography?.fontFamily?.mono);

  const textStyles = t.typography?.text;
  if (textStyles && typeof textStyles === "object") {
    for (const key of Object.keys(textStyles)) {
      const seg = cssSegment(key);
      const st = textStyles[key];
      if (!st || typeof st !== "object") continue;
      set(`typography-text-${seg}-fontSize`, st.fontSize);
      set(`typography-text-${seg}-fontWeight`, st.fontWeight);
      set(`typography-text-${seg}-lineHeight`, st.lineHeight);
      if (typeof st.fontFamily === "string") {
        set(`typography-text-${seg}-fontFamily`, st.fontFamily);
      }
    }
  }

  for (const stop of ["extraSmall", "small", "medium", "large"]) {
    const seg = cssSegment(stop);
    const b = t.typography?.button?.[stop];
    set(`typography-button-${seg}-fontSize`, b?.fontSize);
    set(`typography-button-${seg}-lineHeight`, b?.lineHeight);
  }
}

function emitShadowTokens(t, set) {
  const shadows = t.shadows;
  if (!shadows) return;
  set("shadow-popover", shadows.popover);
  set("shadow-popoverElevated", shadows.popoverElevated);
  set("shadow-sheet", shadows.sheet);
  set("shadow-tooltip", shadows.tooltip);
}

function emitMotionTokens(t, set) {
  emitNestedStringVars(t.motion?.duration, [], "motion-duration", set);
}

export function compareCssVarNames(a, b) {
  const aSpaceMatch = a.match(/^--cube-space-(\d+)px$/);
  const bSpaceMatch = b.match(/^--cube-space-(\d+)px$/);

  if (aSpaceMatch && bSpaceMatch) {
    const diff = Number(aSpaceMatch[1]) - Number(bSpaceMatch[1]);
    return diff !== 0 ? diff : a.localeCompare(b);
  }

  return a.localeCompare(b);
}

export function sortCssVarEntries(vars) {
  return [...vars.entries()].sort(([a], [b]) => compareCssVarNames(a, b));
}

function main() {
  if (!fs.existsSync(INPUT)) {
    console.error(`Missing input: ${INPUT}`);
    process.exit(1);
  }

  const t = JSON.parse(fs.readFileSync(INPUT, "utf8"));

  const vars = new Map();
  const set = (name, value) => {
    if (typeof value === "string") {
      vars.set(`${PREFIX}-${name}`, value);
    } else if (typeof value === "number" && Number.isFinite(value)) {
      vars.set(`${PREFIX}-${name}`, String(value));
    }
  };

  emitNestedStringVars(t.globalColors, [], "color", set);

  emitShadowTokens(t, set);
  emitFoundationSizeTokens(t, set);
  emitTypographyTokens(t, set);
  emitMotionTokens(t, set);

  const focusRing = t.sizes?.focusRing;
  if (focusRing) {
    set("focus-ring-width", focusRing.width);
    set("focus-ring-offset", focusRing.offset);
  }

  const lines = [];
  lines.push("/* AUTO-GENERATED by scripts/build-theme-css.mjs — foundation tokens only */");
  lines.push("/* Component tokens → components.css */");
  lines.push(":root {");
  for (const [k, v] of sortCssVarEntries(vars)) {
    // `.` is not valid in an unescaped CSS custom-property ident (e.g. --cube-space-1.5x) —
    // Chrome silently drops the whole declaration otherwise. Escape it on emission.
    lines.push(`  ${k.replace(/\./g, "\\.")}: ${v};`);
  }
  lines.push("}");

  fs.mkdirSync(path.dirname(OUTPUT), { recursive: true });
  fs.writeFileSync(OUTPUT, lines.join("\n"), "utf8");
  console.log(`Wrote ${OUTPUT}`);
}

const isDirectExecution = process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url;

if (isDirectExecution) {
  main();
}
