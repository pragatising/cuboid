...
| Dialogue | `2484:2462` | Single `Dialog` symbol | Smallest frame in the set — likely just a modal shell. Check whether `Overlay`/`Sheet` (both already exist) already cover this, or whether Dialog is a distinct pattern (e.g. centered vs. Sheet's edge-anchored). **Ask before building** — may be redundant with `Overlay`. |
...
|---|---|---|---|
| Table & Grid | `10006:76590` | `src/components/core/Table/Table.tsx` — structural primitives only: root/header/body/row/cell, density (dense/spacious), line style (none/rows/grid), a generic `SimpleTable<TData>` wrapper. | Figma frame (explicitly noted by user as "editable with a lot more features") specs: 16 cell content types (text, date, label, loading, icon, bar graph, donut chart, checkbox, **inline input**, avatar+name, avatar stack, text+label, empty, smartlink, link+label, switch), sortable header states (asc/desc/hover/focus/pressed), truncated-cell tooltip pattern, row-open button, pagination. This is a large extension, not a new component — scope as its own mini-roadmap once prioritized, likely several sequential PRs (cell-type system first, then sort, then pagination). |
